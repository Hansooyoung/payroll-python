...
def update_karyawan():
    ...
    # Query gabungan (Join) untuk ambil data Profile + Wallet sekaligus
    query = """
        SELECT k.id, k.nama, k.email, k.jabatan_id, k.status_pegawai_id, k.gaji_pokok,
    ...
    if keyword.isdigit():
        ...
        cur.execute(query, (-1, f"%{keyword}%")) # ID -1 dummy
    ...
    # Unpacking Data (Agar kodingan rapi tidak pakai indeks angka)
    k_id, k_nama, k_email, k_jab, k_stat, k_gaji = data[0:6]
    w_id, w_bank, w_rek, w_an = data[6:10] # Bisa None jika belum punya wallet

    print(f"\n[EDIT DATA] Karyawan: {k_nama} (ID: {k_id})")
    print("-" * 50)
    ...
    try:
        ...
        # 2. Upsert Tabel Wallet (Insert or Update)
        if w_id:
            # Jika wallet sudah ada -> UPDATE
            cur.execute("""
            ...
            # Jika belum punya wallet -> INSERT BARU
            if new_bank: # Pastikan bank dipilih
                cur.execute("""
